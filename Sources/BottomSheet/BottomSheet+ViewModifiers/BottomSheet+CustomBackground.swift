...
public extension BottomSheet {
    ...
    ///   - alignment: The alignment with a default value of
    ///     ``Alignment/center`` that you use to position the background view.
    func customFullScreenBackground<Background>(
        _ background: Background,
        alignment: Alignment = .center
    ) -> BottomSheet where Background: View {
        self.configuration.fullScreenBackgroundView = AnyView(background)
        self.configuration.fullScreenBackgroundViewID = UUID()
        return self
    }
    ...
    ///
    /// - Returns: A view that uses the specified content as a background.
    func customFullScreenBackground<V>(
        alignment: Alignment = .center,
        @ViewBuilder content: () -> V
    ) -> BottomSheet where V: View {
        self.configuration.fullScreenBackgroundView = AnyView(content())
        self.configuration.fullScreenBackgroundViewID = UUID()
        return self
    }
    ...
    ///
    /// - Returns: A view with the specified style drawn behind it.
    func customFullScreenBackground<S>(
        _ style: S,
        ignoresSafeAreaEdges edges: Edge.Set = .all
    ) -> BottomSheet where S: ShapeStyle {
        self.configuration.fullScreenBackgroundView = AnyView(
            Rectangle()
                .fill(style)
                .edgesIgnoringSafeArea(edges)
        )
    ...
    ///
    /// - Returns: A view with the specified shape drawn behind it.
    func customFullScreenBackground<S, T>(
        _ style: S,
        in shape: T,
        fillStyle: FillStyle = FillStyle()
    ) -> BottomSheet where S: ShapeStyle, T: Shape {
...
public extension BottomSheet {
    ...
    ///   drawn behind it.
    func customFullScreenBackground(ignoresSafeAreaEdges edges: Edge.Set = .all) -> BottomSheet {
        return self.customFullScreenBackground(
            .background,
